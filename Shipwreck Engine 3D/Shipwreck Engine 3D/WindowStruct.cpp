	...
	switch (elementStructure.formatType)
	{
		case TOP_TO_BOTTOM:
		{
			for (int i = 0; i < elementStructure.elements.size(); i++) {
				...
				startEngine.D3DGfx().DrawRect(Vec2(0.0f, 0.0f), element.size, 19, element.elementRoundness, startEngine.Math().NormalizedColor(0, 255, 0, 255), elementStructure.hWnd);
			}
		}
		break;
